...
const ResourceList = ({ contract, currentUser }) => {
  ...
  const filter = async ({ target }) => {
    ...
    if (checkedState[parseInt(target.id, 10)] === true) {
      console.log("hello");
      setFiltered(true);
      const linkedResources = await contract.getLinkedResources({
  ...
  useEffect(() => {
    ...
    const idSorted = setInterval(() => {
      contract.sortByVoteCount().then((resources) => {
        let sortedResources = resources.slice(
          sliceOffset,
          sliceOffset + PER_PAGE_LIMIT
        );
    ...
    const totalResources = setInterval(() => {
      contract.getResourceCount().then((resourceCount) => {
        setTotalPageCount(
          Array.from(
            { length: Math.ceil(resourceCount / PER_PAGE_LIMIT) },
            (v, k) => k + 1
          )
        );
    ...
    const idCategories = setInterval(() => {
      contract.getCategories().then((categories) => {
                    ...
                    <li>
                      <button
                        className={
                          page === 1
                            ? "invisible"
                            : "h-10 px-5 text-indigo-600 transition-colors duration-150 bg-white rounded-l-lg focus:shadow-outline hover:bg-indigo-100"
                        }
                    ...
                    {totalPageCount.map((index) => {
                      return (
                        <li>
                          <button
                            key={index}
                            className="h-10 px-5 text-white transition-colors duration-150 bg-indigo-600 focus:shadow-outline"
